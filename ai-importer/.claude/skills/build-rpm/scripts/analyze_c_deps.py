...
依赖来源：
  ...
  3. meson.build    — dependency() / cc.find_library()
  4. Makefile       — -l<lib> 链接标志（兜底）

RPM 三级查询策略（共享一次性批量查询，无需映射表）：
  Level 1: `pkgconfig(foo)`
  Level 2: `cmake(Foo)`
  Level 3: `libfoo*-devel` / `foo*-devel` 包名回退

用法：
  python3 analyze_c_deps.py <source_dir>
  python3 analyze_c_deps.py <source_dir> --check-rpm --container oe-build-env
  python3 analyze_c_deps.py <source_dir> --check-rpm --container oe-build-env -o result.json
"""
...
GLIBC_BUILTINS = {"pthread", "m", "dl", "c", "rt", "gcc_s", "resolv", "util"}

# 纯构建辅助，不需要查 RPM
CMAKE_SKIP = {
    "CMakePackageConfigHelpers", "GNUInstallDirs", "PackageHandleStandardArgs",
    "CheckCCompilerFlag", "CheckIncludeFile", "CheckFunctionExists",
    "CheckLibraryExists", "FeatureSummary", "CTest", "CPack",
    "ExternalProject", "FetchContent",
    # cmake 内置伪目标，无对应 RPM 包
    "Threads", "PkgConfig", "Python", "Python3", "Python2",
    "FindPkgConfig", "Colors", "Git",
}
...
def parse_cmake(source_dir: str) -> Dict:
    ...
    for cmake_file in list(src.rglob("CMakeLists.txt")) + list(src.rglob("*.cmake")):
        ...
        for m in re.finditer(r"find_package\s*\(\s*(\w+)([^)]*)\)", content, re.IGNORECASE | re.DOTALL):
            ...
            find_items.append(build_dependency_item("find_package", pkg, requirement))
...
def parse_meson(source_dir: str) -> Dict:
    """解析 meson.build，提取 dependency() 和 cc.find_library()。"""
    src = Path(source_dir)
    pkg_items: List[Dict[str, str]] = []
    link_items: List[Dict[str, str]] = []

    for meson_file in src.rglob("meson.build"):
        try:
            content = meson_file.read_text(errors="ignore")
        except OSError:
            continue

        for m in re.finditer(r"dependency\s*\(\s*['\"]([^'\"]+)['\"]([^)]*)\)", content, re.IGNORECASE | re.DOTALL):
            name = m.group(1)
            if name in GLIBC_BUILTINS:
                continue
            tail = m.group(2)
            requirement = ""
            version_match = re.search(r"version\s*:\s*['\"]([^'\"]+)['\"]", tail)
            if version_match:
                requirement = normalize_requirement(version_match.group(1))
            pkg_items.append(build_dependency_item("pkgconfig", name, requirement))

        for m in re.finditer(r"find_library\s*\(\s*['\"]([^'\"]+)['\"]", content):
            lib = m.group(1)
            if lib not in GLIBC_BUILTINS:
                link_items.append(build_dependency_item("link", lib))
    ...
    pkg_items = merge_dependency_items(pkg_items)
    link_items = merge_dependency_items(link_items)
    dependency_items = merge_dependency_items(pkg_items + link_items)
...
def build_lookup_tasks(parsed: Dict) -> List[Dict]:
    ...
    for item in parsed.get("find_package_items", []):
        ...
        tasks.append({
            **item,
            "queries": [
                provides_query(f"pkgconfig({pkg.lower()})", "pkgconfig()"),
    ...
    for item in parsed.get("pkg_module_items", []):
        ...
        tasks.append({
            **item,
            "queries": [
                provides_query(f"pkgconfig({pc.lower()})", "pkgconfig()"),
        ...
        tasks.append({
            **item,
            ...
            "queries": [
                ...
                name_glob_query(f"lib{lib_lower}*-devel", "name-glob", prefer_devel=True),
                name_glob_query(f"{lib_lower}*-devel", "name-glob", prefer_devel=True),
            ],
        })
    return tasks
...
def check_rpm_availability(parsed: Dict = None) -> Dict:
    """
    本地用 dnf.Base().fill_sack() 加载一次元数据，然后内存批量查询所有依赖。
    比每次调用 dnf repoquery 子进程快很多，并统一各语言 fallback 逻辑。
    """
    if parsed is None:
        parsed = {}
    ...
    print(f"\n[INFO] 本地查询 RPM 可用性（dnf sack，共 {len(tasks)} 个依赖）...")

    try:
        results = run_batch_lookup(tasks, timeout=120)
    except (BatchLookupError, OSError, json.JSONDecodeError) as e:
        print(f"[WARN] dnf sack 查询失败（{e}），跳过依赖检查")
        results = fallback_results(tasks)

    available, missing = [], []
    for item in results:
        dep, dep_type, rpm, level = item["dep"], item["type"], item.get("rpm"), item.get("level", "")
...
def build_rpm_requires(build_system: str, rpm_check: Optional[Dict]) -> List[str]:
    ...
    if build_system == "cmake":
        ...
        result += ["autoconf", "automake", "libtool"]
...
def print_report(parsed: Dict, rpm_check: Optional[Dict]):
    ...
    print_items("find_package / dependency 依赖", parsed.get("find_package_items", []))
...
def main():
    parser = argparse.ArgumentParser(description="C 包 RPM 依赖分析")
    parser.add_argument("source_dir", help="C 项目源码目录")
